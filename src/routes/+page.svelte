	...
	let filtered = $derived(
		people.filter((p) => {
			...
			return (
				...
				p.roles.some((r) => r.role.toLowerCase().includes(needle))
			);
		})
	);

	const link = (p: { id: number; name: string; roles: { role: string }[] }) =>
		p.roles.find((r) => r.role === 'member') ? `/m/${p.name.toLowerCase()}` : `/m/${p.id}`;
...
<PageShell>
	<div class="head">
		<Tabs>
			<Tab
				active={!showAll}
				count={people.filter((p) => p.roles.some((r) => r.role === 'member')).length}
			...
			<DataTable>
				...
				{#snippet body()}
					{#each filtered as p (p.id)}
						<tr class="clickable" onclick={() => goto(link(p))}>
