...
- **Nav links** (only shown when logged in):
  - Members → `/`
  - Mail → `/mail` *(board only)*
  - Changes → `/changes` *(board only)*
  - Own name → `/m/{name}`
  - Logout → `/auth/logout`
...
- **"Create" button** → `/m/+new` *(board only, shown top-left)*
...
### `/m/+new` — Create member *(board only)*
